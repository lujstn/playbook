...
fail=0
echo "== bash -n lint ==";   for f in "$root"/../hooks/session-start "$root"/../hooks/take-a-beat "$root"/../hooks/comment-guard "$root"/../hooks/notification "$root"/../hooks/dispatch-guard "$root"/../hooks/lib/playbook-common.sh "$root"/../scripts/offline; do bash -n "$f" && echo "ok $f" || fail=1; done
...
for ev in Stop SubagentStop PostToolUse PreCompact Notification; do
  for hk in session-start take-a-beat comment-guard notification dispatch-guard; do
