...
echo "== long-dash sweep =="; sweep_targets=("$root/../skills" "$root/../hooks" "$root/../docs/playbook" "$root/../.claude-plugin" "$root/../design.md"); [ -e "$root/../README.md" ] && sweep_targets+=("$root/../README.md"); if grep -rlnP '\x{2014}|\x{2013}' "${sweep_targets[@]}" 2>/dev/null; then echo "FAIL: long dash (U+2014 or U+2013) found in shipped/spec surface"; fail=1; else echo "ok: no long dashes"; fi
