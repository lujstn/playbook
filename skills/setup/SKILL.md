...
Also list any standalone commands already under `~/.claude/commands/` (`ls ~/.claude/commands/*.md`). Their basenames are the only thing that can collide with Playbook's bare aliases (`brainstorming`, `debug`, `fix`, `offline-mode`, `worktrees`, `hello`, `workflow`), because every plugin command is namespaced and a plugin can never own a bare name. A collision is not a fault to repair; it just means that one bare alias is left untouched and the `/playbook:<name>` form covers it. You use this list in the alias offer in step 4.
...
- **Bare-name collisions** are handled by the alias offer in step 4, not judged here. Where a bare name is already taken, that one alias is skipped and the `/playbook:<name>` form covers it; nothing is ever overwritten.
...
- **Offline notifications**: check whether a channel is set up (a `notify-provider` or `ntfy-topic` resolving from `~/.claude/playbook/` or the project's `.claude/playbook/`). If one is, leave it. If not, and the user has not already declined (`~/.claude/playbook/notify` does not read `skipped`), offer to set one up now as yes / not now / never. On **yes**, follow the guided walkthrough in `../offline-mode/notify-setup.md` (the offline-mode skill's directory): it asks iPhone or Android, explains ntfy (free) versus Pushover (a one-off cost), configures the chosen one globally under `~/.claude/playbook/`, offers an Android QR, and sends a live test ping. On **not now**, move on and it is offered again another time. On **never**, write `skipped` to `~/.claude/playbook/notify`. Deferring is a perfectly good answer.
- **Bare command aliases**: Claude Code always namespaces a plugin's commands, so out of the box only the `/playbook:<name>` forms exist; a plain `/offline-mode` cannot come from the plugin itself. Playbook can install the bare `/brainstorming`, `/debug`, `/fix`, `/offline-mode`, `/worktrees`, `/hello` and `/workflow` as standalone files under `~/.claude/commands/`, each pointing straight back at the plugin. Read `~/.claude/playbook/aliases` to decide whether to raise it: absent means this is the first offer; a `version=` line differing from the plugin's current version means offer a refresh instead; `skipped` or `removed` means stay quiet unless the user asks. When you do raise it, offer yes / not now / never. On **yes**, run the installer at `${CLAUDE_PLUGIN_ROOT}/scripts/install-aliases.sh` (if that variable is not set in your shell, resolve the path with `jq -r 'first(.plugins | to_entries[] | select(.key|test("^playbook@")) | .value[0].installPath) // empty' ~/.claude/plugins/installed_plugins.json` and append `/scripts/install-aliases.sh`), then relay its per-name lines plainly: it installs each name it can and, where one is already owned by another tool or by the user, skips just that name and says so, leaving `/playbook:<name>` as the way in. Mention that new commands appear after restarting Claude Code. On **not now**, move on and it is offered again another time. On **never**, write `skipped` to `~/.claude/playbook/aliases`. Removal later is the same script with `--remove`, which only ever deletes files Playbook wrote.
